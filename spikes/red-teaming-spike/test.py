...
async def main():
        ...
        try:
            ...
            if "messages" in json_response and len(json_response["messages"]) > 1:
                ...
                if "content" in ai_message:
                    ...
                    print(
                        f"DEBUG: PyRIT Callback - Extracted response: {response_to_adversarial_chat[:100]}..."
                    )
            ...
            if "output" in json_response and json_response["output"] != "":
                ...
                if "displayResponse" in response_to_adversarial_chat:
                    ...
                    if (
                        "recommendedProducts" in json_response["output"]
                        and json_response["output"]["recommendedProducts"] is not None
                    ):
            ...
            if hasattr(e, "response"):
                print(
                    f"DEBUG: PyRIT Callback - HTTP response: {e.response.status_code if hasattr(e.response, 'status_code') else 'N/A'}"
                )
                print(
                    f"DEBUG: PyRIT Callback - HTTP response text: {e.response.text if hasattr(e.response, 'text') else 'N/A'}"
                )
    ...
    red_team_agent = RedTeam(
        azure_ai_project=azure_foundry_endpoint,  # azure_ai_project, # required
    ...
    try:
        ...
        if hasattr(red_team_result, "attack_details"):
            print(
                f"DEBUG: attack_details length: {len(red_team_result.attack_details) if red_team_result.attack_details else 0}"
            )
    ...
    if hasattr(red_team_result, "risk_category_asr") and red_team_result.risk_category_asr is not None:
        ...
        if hasattr(red_team_result, "get") and red_team_result.get("risk_category_asr") is not None:
            print(
                f"Attempting to get 'risk_category_asr' as a key: {red_team_result.get('risk_category_asr')}"
            )

    # Print a summary of results by attack complexity
    print("\\nResults by Attack Complexity:")
    if (
        hasattr(red_team_result, "attack_complexity_asr")
        and red_team_result.attack_complexity_asr is not None
    ):
        ...
        if hasattr(red_team_result, "get") and red_team_result.get("attack_complexity_asr") is not None:
            print(
                f"Attempting to get 'attack_complexity_asr' as a key: {red_team_result.get('attack_complexity_asr')}"
            )
