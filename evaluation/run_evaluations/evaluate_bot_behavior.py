...
def get_evaluator_configs(config: AzureOpenAIModelConfiguration):
    ...
    evaluator_configs.append_config("bot_behavior_display", BotBehaviorEvaluator(config), {
        "question": "${data.question}", "answer": "${target.answer}", "expected_behavior": "${data.expected_behavior}"})
    evaluator_configs.append_config("bot_behavior_voice", BotBehaviorEvaluator(config), {
        "question": "${data.question}", "answer": "${target.voice_summary}", "expected_behavior": "${data.expected_behavior}"})
...
def generate_evaluation_result(df):
    ...
    failing_records = df[(df["outputs.bot_behavior_display.score"] < 4) |
                         (df["outputs.bot_behavior_voice.score"] < 3)][[
                             "outputs.question",
                             "outputs.answer",
