...
from evaluators import (CalledToolEvaluator, CoherenceEvaluator,
...
from tests.test_utils.evalutation_test_utils import (validate_evaluator,
...
class TestEvalFullFlow(unittest.TestCase):
    ...
    def evaluate_tester(self, target, evaluation_name, data, evaluators, evaluator_config):
        return {"target": target, "evaluation_name": evaluation_name, "data": data, "evaluators": evaluators, "evaluator_config": evaluator_config}

    def test_evaluators(self):

        result = evaluate_full_flow(dataset_path="path", model_config=self.model_config,
                                    evaluate_function=self.evaluate_tester, ignore_environment_validation=True
                                    )
        ...
        validate_evaluator(
            self, evaluators, "json_schema_validation", JsonSchemaValidationEvaluator)
        validate_evaluator(self, evaluators, "voice_summary_groundedness",
                           RAGGroundednessEvaluator)
        validate_evaluator(self, evaluators, "display_response_groundedness",
                           RAGGroundednessEvaluator)
        validate_evaluator(
            self, evaluators, "display_response_fluency", FluencyEvaluator)
        validate_evaluator(
            self, evaluators, "display_response_coherence", CoherenceEvaluator)
        validate_evaluator(
            self, evaluators, "voice_summary_fluency", FluencyEvaluator)
        validate_evaluator(
            self, evaluators, "voice_summary_coherence", CoherenceEvaluator)
        validate_evaluator(self, evaluators, "relevance",
                           RelevanceOptionalContextEvaluator)
        ...
        validate_evaluator_config(
            self, evaluator_config, "relevance", "question", "${data.question}")
        validate_evaluator_config(
