...
class MockRunnableFactory:
    def get_runnable(self) -> Runnable:
        simple_runnable = RunnableLambda(lambda question_input: {
...
environment_config = EnvironmentConfig(
    anonymize_input=True, anonymizer_mode="CUSTOM")
app_settings = AppSettings(environment_config=environment_config)
runnable_factory = MockRunnableFactory()
app = AppFactory(app_settings=app_settings,
                 runnable_factory=runnable_factory).app
...
class TestApp(unittest.TestCase):
    def test_anonymizer_is_called(self):
        ...
        payload = {"input": {
            "question": "hello there my phone number is 512-111-1111"}, "config": config}

        response = client.post("/agent/invoke", json=payload)

        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "hello there my phone number is redacted_value", response.text)
